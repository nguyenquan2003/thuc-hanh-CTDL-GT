...
void DocFile(SINHVIEN sv[MAXSIZE], int &n)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		fgets(sv[i].HOSV, 30, f); 
		sv[i].HOSV[strlen(sv[i].HOSV) - 1] = '\0';
		fgets(sv[i].TENSV, 10, f); 
...
void XuatDS(SINHVIEN sv[MAXSIZE], int n)
{
	...
	printf("\n%10s %20s %10s %10s", "MSSV", "HO","TEN", "DIEMTB");
	for (int i = 0; i<n; i++)
	{
		printf("\n%10s %20s %10s %10.1f", sv[i].MASV, sv[i].HOSV,sv[i].TENSV, sv[i].DiemTB);
	}
...
void XuatSV_DTBMAX(SINHVIEN sv[MAXSIZE], int n)
{
	for (int i = 0; i < n; i++)
	{
		...
		{
			printf("\n%10s %15s %10s %12s", "MSSV", "HO", "TEN", "DIEMTB");
			printf("\n%10s %15s %10s %10.1f", sv[i].MASV, sv[i].HOSV, sv[i].TENSV, sv[i].DiemTB);
...
{
	printf("\n%10s %15s %10s %12s", "MSSV", "HO", "TEN", "DIEMTB");
	printf("\n%10s %15s %10s %10.1f", sv.MASV, sv.HOSV, sv.TENSV, sv.DiemTB);
